...
export default function Avatar({ waveHand, setWaveHand, ...props }) {
  ...
  useEffect(() => {
    if (waveHand) {
      setCurrentAnimation("wave_hand");

      const interval = setInterval(() => {
        setCurrentAnimation("idle");
        setWaveHand(false);
      }, 2000); //change to idle after 2s
  ...
  useEffect(() => {
    if (currentAnimation == "idle") {
      actions[currentAnimation].reset().fadeIn(0.8).play();
      return () => {
        actions[currentAnimation].fadeOut(0.8);
      };
    }
    actions[currentAnimation].reset().fadeIn(0.8).play().setLoop(LoopOnce);
    return () => actions[currentAnimation].fadeOut(0.8);
  }, [currentAnimation]);
  ...
  const changeAnimation = () => {
    ...
    setCurrentAnimation(ACTION_LIST[action_id]);
  };

  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentAnimation("idle");
    }, 10000); //change to idle after 10s
